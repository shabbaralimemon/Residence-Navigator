...
const Conversations = ({ conversationInfo }) => {
    const { conversation={chatPartner:{}}, trackConversation, setTrackConversation } = conversationInfo;
    ...
    // Ensure that conversation and its properties are available
                ...
                conversation.participantId && conversation.participantId === currentUser._id
                    ? (
                        ...
                        >
                            ...
                            <p className='hidden sm:block font-semibold font-content text-sm truncate duration-400 ease-in-out group-hover:text-black'>{conversation.chatCreator.username}</p>
                        ...
                        <div
                            ...
                            className={`chat_user flex items-center justify-center sm:justify-start sm:flex-row sm:gap-4 hover:bg-brand-blue/90 active:bg-brand-blue group w-full p-2 sm:p-3 gap-1 duration-300 cursor-pointer ${trackConversation.conversationActive === conversation.chatPartner._id ? "bg-brand-blue text-white" : "bg-gray-200 text-brand-blue"}`}
                        >
                            <div className='relative rounded-full'>
                                <img
                                    className='h-8 w-8 sm:h-12 sm:w-12 rounded-full border-2 border-brand-blue duration-400 ease-in-out'
                            ...
                            <p className='hidden sm:block font-semibold font-content text-sm truncate duration-400 ease-in-out group-hover:text-black'>{conversation.chatPartner.username}</p>
                            {isNotify && <p className='bg-red-600 hidden md:block font-heading px-2 py-[2px] rounded-md capitalize text-xs text-black'>new!</p>}
                            {isNotify && <p className='text-red-600 md:hidden font-heading rounded-md capitalize text-lg'>!</p>}
                        </div>
                    )
            }
